...
required from medium model \""     + mediumName + "\".");
  ...
  /*
    assert(Xi[Water] <= X_sat, "The medium model '" + mediumName + "' must not be saturated.\n"
     + "To model a saturated medium, use 'Buildings.Obsolete.Media.GasesPTDecoupled.MoistAir' instead of this medium.\n"
     + " T         = " + String(T) + "\n"
     + " X_sat     = " + String(X_sat) + "\n"
...
algorithm
  ...
  annotation(Inline=false,smoothOrder=5,derivative=Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.saturationPressureLiquid_der,
...
algorithm
  h := (T-273.15) * steam.cp + Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfVaporization(T);
...
algorithm
  h := Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfCondensingGas(T)*X[Water]
       + Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfDryAir(T)*(1.0-X[Water]);
end enthalpyOfGas;
...
algorithm
  h := Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p, state.T, state.X);
end specificEnthalpy;

redeclare function extends specificInternalEnergy "Specific internal energy"
  extends Modelica.Icons.Function;
algorithm
  u := Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p,state.T,state.X) - pStp/dStp;
end specificInternalEnergy;

redeclare function extends specificGibbsEnergy "Specific Gibbs energy"
  extends Modelica.Icons.Function;
algorithm
  g := Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p,state.T,state.X) - state.T*specificEntropy(state);
end specificGibbsEnergy;

redeclare function extends specificHelmholtzEnergy "Specific Helmholtz energy"
  extends Modelica.Icons.Function;
algorithm
  f := Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p,state.T,state.X)
         - gasConstant(state)*state.T
         - state.T*Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.specificEntropy(state);
...
<a href=\"modelica://Buildings.Obsolete.Media.GasesPTDecoupled.MoistAir\">
Buildings.Obsolete.Media.GasesPTDecoupled.MoistAir</a>,  but 
in this model, the air must not be saturated. If the air is saturated, 
use the medium model
<a href=\"modelica://Buildings.Obsolete.Media.GasesPTDecoupled.MoistAir\">
Buildings.Obsolete.Media.GasesPTDecoupled.MoistAir</a> instead of this one.
</p>
<p>
...
<a href=\"modelica://Buildings.Obsolete.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfNonCondensingGas\">
